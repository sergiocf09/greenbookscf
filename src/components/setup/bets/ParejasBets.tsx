...
export const ParejasBets: React.FC<ParejasBetsProps> = ({
  ...
  return (
    <div className="space-y-3">
      ...
      <BetSection
        id="sixesSetup"
        title="Sixes / Round Robin"
        description="Parejas rotativas en 3 sets de 6 hoyos"
        enabled={config.sixesSetup?.enabled ?? false}
        onToggle={(enabled) => {
          onUpdateBet('sixesSetup', { enabled } as any);
          if (enabled) onToggleSection('sixesSetup', true);
        }}
        isExpanded={expandedSections.includes('sixesSetup')}
        onExpandChange={(open) => onToggleSection('sixesSetup', open)}
        helpText="Tres sets de 6 hoyos (1-6, 7-12, 13-18) con parejas que rotan cada set. Se compara score neto por equipo y se cobra por set ganado o por hoyo ganado."
      >
        <div className="space-y-3">
          {/* Scoring mode */}
          <div className="flex items-center justify-between">
            ...
            <Select
              value={config.sixesSetup?.scoringMode ?? 'lowBall'}
              onValueChange={(v: 'lowBall' | 'lowHighBall' | 'stroke') => onUpdateBet('sixesSetup', { scoringMode: v } as any)}
            >
              <SelectTrigger className="h-7 w-28 text-[11px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="lowBall">Mejor Bola</SelectItem>
                <SelectItem value="lowHighBall">Mejor+Peor</SelectItem>
          ...
          <div className="space-y-1">
            ...
            <RadioGroup
              value={config.sixesSetup?.cobro ?? 'per_hole'}
              onValueChange={(v: 'per_set' | 'per_hole') => onUpdateBet('sixesSetup', { cobro: v } as any)}
              className="flex gap-3"
            >
              <div className="flex items-center gap-1.5">
                <RadioGroupItem value="per_set" id="sixes-per-set" />
                <Label htmlFor="sixes-per-set" className="text-[10px]">Por set</Label>
              </div>
              <div className="flex items-center gap-1.5">
                <RadioGroupItem value="per_hole" id="sixes-per-hole" />
                <Label htmlFor="sixes-per-hole" className="text-[10px]">Por hoyo</Label>
              </div>
            </RadioGroup>
          </div>
          ...
          {/* Amount */}
          <AmountInput
            label={(config.sixesSetup?.cobro ?? 'per_hole') === 'per_set' ? 'Monto por set' : 'Monto por hoyo'}
            value={config.sixesSetup?.amount ?? 100}
            onChange={(v) => onUpdateBet('sixesSetup', { amount: v } as any)}
          />

          {/* Handicap toggle */}
          <div className="flex items-center justify-between">
            <Label className="text-[10px] text-muted-foreground">Usar hándicap</Label>
            <Switch
              checked={config.sixesSetup?.useHandicap ?? true}
              onCheckedChange={(v) => onUpdateBet('sixesSetup', { useHandicap: v } as any)}
            />
          </div>
        </div>
      </BetSection>
