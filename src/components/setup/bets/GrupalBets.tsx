          ...
          <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onUpdateBet('coneja', { handicapMode: 'individual' as ConejaHandicapMode });
              }}
              className={cn(
            ...
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onUpdateBet('coneja', { handicapMode: 'bilateral' as ConejaHandicapMode });
              }}
              className={cn(
                "flex-1 px-3 py-2 text-xs rounded transition-colors border",
      ...
      >
        ...
        <div className="mt-3 p-3 bg-muted/30 rounded-lg space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <PointInput 
              label="Albatros" 
              value={config.stableford?.points?.albatross ?? 5}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, albatross: v } })}
            />
            <PointInput 
              label="Águila" 
              value={config.stableford?.points?.eagle ?? 4}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, eagle: v } })}
            />
            <PointInput 
              label="Birdie" 
              value={config.stableford?.points?.birdie ?? 3}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, birdie: v } })}
            />
            <PointInput 
              label="Par" 
              value={config.stableford?.points?.par ?? 2}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, par: v } })}
            />
            <PointInput 
              label="Bogey" 
              value={config.stableford?.points?.bogey ?? 1}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, bogey: v } })}
            />
            <PointInput 
              label="Doble Bogey" 
              value={config.stableford?.points?.doubleBogey ?? 0}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, doubleBogey: v } })}
            />
            <PointInput 
              label="Triple Bogey" 
              value={config.stableford?.points?.tripleBogey ?? -1}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, tripleBogey: v } })}
            />
            <PointInput 
              label="Cuádruple+" 
              value={config.stableford?.points?.quadrupleOrWorse ?? -2}
              onChange={(v) => onUpdateBet('stableford', { points: { ...config.stableford.points, quadrupleOrWorse: v } })}
            />
          </div>
        </div>
