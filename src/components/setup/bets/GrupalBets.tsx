    ...
    <div className="space-y-3">
      ...
      >
        <AmountInput 
          ...
          onChange={(v) => onUpdateBet('coneja', { amount: v })} 
        />
        
        <CollapsibleSubSection
          label="Configuración"
          summary={`Handicap: ${(config.coneja?.handicapMode ?? 'individual') === 'individual' ? 'USGA' : 'Sliding'}`}
        >
          <div className="space-y-3">
            ...
            <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onUpdateBet('coneja', { handicapMode: 'individual' as ConejaHandicapMode }); }}
                className={cn(
              ...
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onUpdateBet('coneja', { handicapMode: 'bilateral' as ConejaHandicapMode }); }}
                className={cn(
                  "flex-1 px-3 py-2 text-xs rounded transition-colors border",
      ...
      <BetSection
        ...
        onToggle={(enabled) => onUpdateBet('culebras', { enabled })}
        isExpanded={expandedSections.includes('culebras')}
        onExpandChange={(open) => onToggleSection('culebras', open)}
        color="red"
      >
      ...
      <BetSection
        id="pinguinos"
        title="Pingüinos 🐧"
        description="Triple bogey o peor (bruto vs par), el último paga todas"
        enabled={config.pinguinos.enabled}
        onToggle={(enabled) => onUpdateBet('pinguinos', { enabled })}
        isExpanded={expandedSections.includes('pinguinos')}
        onExpandChange={(open) => onToggleSection('pinguinos', open)}
        color="red"
      >
      ...
      >
        ...
        <CollapsibleSubSection
          label="Configuración"
          summary={`${(config.zoologico?.enabledAnimals ?? ['camello', 'pez', 'gorila']).length} animales`}
        >
      ...
      <BetSection
        ...
        onToggle={(enabled) => {
          ...
          if (enabled && currentHandicaps.length === 0) {
            ...
            onUpdateBet('medalGeneral', { enabled });
          }
        }}
        isExpanded={expandedSections.includes('medalGeneral')}
        onExpandChange={(open) => onToggleSection('medalGeneral', open)}
        color="gold"
      >
        <AmountInput 
          label="Cantidad por jugador" 
          value={config.medalGeneral?.amount ?? 100} 
          onChange={(v) => onUpdateBet('medalGeneral', { amount: v })} 
        />
        
        <CollapsibleSubSection
          label="Configuración"
          summary="Handicaps por jugador"
        >
          <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-1">
                    <Button
                      ...
                      onClick={(e) => {
                        ...
                        const newHandicaps = existingHandicaps.map(pc => pc.playerId === player.id ? { ...pc, handicap: Math.max(0, pc.handicap - 1) } : pc);
                        if (!newHandicaps.some(pc => pc.playerId === player.id)) newHandicaps.push({ playerId: player.id, handicap: Math.max(0, player.handicap - 1) });
                        onUpdateBet('medalGeneral', { playerHandicaps: newHandicaps });
                      }}
                    ...
                    <Input type="number" value={currentHcp}
                      onChange={(e) => {
                        ...
                        const existingHandicaps = config.medalGeneral?.playerHandicaps || [];
                        const newHandicaps = existingHandicaps.map(pc => pc.playerId === player.id ? { ...pc, handicap: newValue } : pc);
                        if (!newHandicaps.some(pc => pc.playerId === player.id)) newHandicaps.push({ playerId: player.id, handicap: newValue });
                        onUpdateBet('medalGeneral', { playerHandicaps: newHandicaps });
                      }}
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        const existingHandicaps = config.medalGeneral?.playerHandicaps || [];
                        const newHandicaps = existingHandicaps.map(pc => pc.playerId === player.id ? { ...pc, handicap: pc.handicap + 1 } : pc);
                        if (!newHandicaps.some(pc => pc.playerId === player.id)) newHandicaps.push({ playerId: player.id, handicap: player.handicap + 1 });
                        onUpdateBet('medalGeneral', { playerHandicaps: newHandicaps });
                      }}
      ...
      <BetSection
        id="stableford"
        title="Stableford 📊"
        description="Grupal: puntos por score relativo al par"
        enabled={config.stableford?.enabled ?? false}
        onToggle={(enabled) => {
          if (enabled && config.stableford.playerHandicaps.length === 0) {
            const initialHandicaps: StablefordPlayerConfig[] = players.map(p => ({
              playerId: p.id,
              handicap: p.handicap,
            }));
            onUpdateBet('stableford', { 
              enabled, 
              playerHandicaps: initialHandicaps,
              points: config.stableford.points || DEFAULT_STABLEFORD_POINTS,
            });
          } else {
            onUpdateBet('stableford', { enabled });
          }
        }}
        isExpanded={expandedSections.includes('stableford')}
        onExpandChange={(open) => onToggleSection('stableford', open)}
        color="gold"
      >
        <AmountInput 
          label="Cantidad por jugador" 
          value={config.stableford?.amount ?? 100} 
          onChange={(v) => onUpdateBet('stableford', { amount: v })} 
        />
        
        <CollapsibleSubSection
          label="Configuración"
          summary="Puntos y Handicaps"
        >
          <div className="space-y-3">
            {/* Point values */}
            ...
            <div className="space-y-2">
              ...
              {players.map(player => {
                ...
                return (
                  <div key={player.id} className="flex items-center justify-between gap-2 p-2 bg-muted/30 rounded">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white" style={{ backgroundColor: player.color }}>
                        {player.initials}
                      </div>
                      <span className="text-xs font-medium">{formatPlayerName(player.name)}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button type="button" variant="outline" size="icon" className="h-6 w-6"
                        onClick={(e) => {
                          e.stopPropagation();
                          const existingHandicaps = config.stableford?.playerHandicaps || [];
                          const newHandicaps = existingHandicaps.map(pc => pc.playerId === player.id ? { ...pc, handicap: Math.max(0, pc.handicap - 1) } : pc);
                          if (!newHandicaps.some(pc => pc.playerId === player.id)) newHandicaps.push({ playerId: player.id, handicap: Math.max(0, player.handicap - 1) });
                          onUpdateBet('stableford', { playerHandicaps: newHandicaps });
                        }}
                      ><Minus className="h-3 w-3" /></Button>
                      <Input type="number" value={currentHcp}
                        onChange={(e) => {
        ...
        <p className="text-[9px] text-muted-foreground mt-2">
          El ganador con más puntos Stableford cobra a los demás.
        </p>
      </BetSection>
    </div>
  );
