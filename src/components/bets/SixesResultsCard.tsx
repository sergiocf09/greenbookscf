            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-center gap-1">
                  <span className="text-xs font-bold" style={{ color: players.find(p => p.id === myTeam[0])?.avatarColor }}>{disambiguated.get(myTeam[0]) ?? '?'}</span>
                  <span className="text-[9px] text-muted-foreground">/</span>
                  <span className="text-xs font-bold" style={{ color: players.find(p => p.id === myTeam[1])?.avatarColor }}>{disambiguated.get(myTeam[1]) ?? '?'}</span>
                </div>
                <div className="text-[10px] text-muted-foreground text-center">vs</div>
                <div className="flex items-center justify-center gap-1">
                  <span className="text-xs font-bold" style={{ color: players.find(p => p.id === rivalTeam[0])?.avatarColor }}>{disambiguated.get(rivalTeam[0]) ?? '?'}</span>
                  <span className="text-[9px] text-muted-foreground">/</span>
                  <span className="text-xs font-bold" style={{ color: players.find(p => p.id === rivalTeam[1])?.avatarColor }}>{disambiguated.get(rivalTeam[1]) ?? '?'}</span>
                </div>
