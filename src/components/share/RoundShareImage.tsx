  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-center">{trs("Compartir Resultado")}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-2 pt-2">
          ...
          {showFallbackInstructions && (
            <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 p-3 text-sm text-amber-800 dark:text-amber-200 text-center">
              {trs("✅ Imagen guardada en tu galería. Abre WhatsApp, selecciona el chat y adjunta la imagen desde tu galería de fotos.")}
            </div>
          )}
