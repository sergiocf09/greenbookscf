-- Update create_round function to accept starting_hole parameter
CREATE OR REPLACE FUNCTION public.create_round(
  p_course_id uuid, 
  p_tee_color text, 
  p_date date, 
  p_bet_config jsonb,
  p_starting_hole integer DEFAULT 1
)
...
BEGIN
  ...
  VALUES (p_course_id, v_profile_id, COALESCE(p_tee_color, 'white'), COALESCE(p_date, CURRENT_DATE), 'setup', COALESCE(p_bet_config, '{}'::jsonb), COALESCE(p_starting_hole, 1))
