-- Add configurable points per match
ALTER TABLE public.cup_matches 
  ADD COLUMN IF NOT EXISTS points_per_match numeric NOT NULL DEFAULT 1;

-- Drop old function so we can change return signature
DROP FUNCTION IF EXISTS public.get_cup_match_result(uuid);

-- Recreate with hole_breakdown jsonb
CREATE OR REPLACE FUNCTION public.get_cup_match_result(p_match_id uuid)
RETURNS TABLE(
  holes_played integer,
  holes_remaining integer,
  side_a_holes_won integer,
  side_b_holes_won integer,
  current_standing text,
  result_type text,
  match_closed boolean,
  hole_breakdown jsonb
)
LANGUAGE plpgsql
SECURITY DEFINER
