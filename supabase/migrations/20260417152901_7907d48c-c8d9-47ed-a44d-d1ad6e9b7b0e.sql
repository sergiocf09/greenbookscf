...
BEGIN
  ...
  ORDER BY
    -- Primero los que ya tienen hoyos jugados, ordenados por mejor score (asc)
    CASE WHEN COUNT(DISTINCT hs.hole_number) = 0 THEN 1 ELSE 0 END,
    COALESCE(SUM(hs.strokes - ch.par), 0) ASC,
    COUNT(DISTINCT hs.hole_number) DESC,
    p.display_name;
END;
$function$;