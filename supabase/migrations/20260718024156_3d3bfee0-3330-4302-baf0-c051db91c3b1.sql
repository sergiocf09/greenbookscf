...
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Resumen de Ronda · GreenBook CF</title></head>
...
<tr><td style="background:linear-gradient(135deg,#0f4c2a 0%%,#0a2d18 50%%,#061a0e 100%%);padding:28px 24px 20px;border-radius:16px 16px 0 0;text-align:center;">
<div style="font-size:11px;font-weight:700;letter-spacing:3px;color:#4ade80;text-transform:uppercase;margin-bottom:6px;">GreenBook CF</div>
...
<td style="text-align:center;padding:0 8px;"><div style="font-size:11px;color:#64748b;margin-bottom:4px;">H''CAP</div><div style="font-size:36px;font-weight:800;line-height:1;color:%s;">%s<span style="font-size:16px;">%s</span></div></td>
</tr></table></td></tr>
...
<div style="font-size:11px;color:#374151;">GreenBook CF · golfgreenbookscf.com</div>
<div style="font-size:10px;color:#1f2937;margin-top:4px;">© 2026 GreenBook CF. Todos los derechos reservados.</div>
...
GRANT EXECUTE ON FUNCTION public.enqueue_round_close_emails(uuid) TO service_role;

CREATE OR REPLACE FUNCTION public.trg_fn_round_close_emails()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF NEW.status = 'completed' AND OLD.status <> 'completed'
     AND COALESCE(NEW.is_incomplete, false) = false THEN
    PERFORM public.enqueue_round_close_emails(NEW.id);
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_round_close_emails ON public.rounds;
CREATE TRIGGER trg_round_close_emails
  AFTER UPDATE OF status ON public.rounds
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_fn_round_close_emails();
